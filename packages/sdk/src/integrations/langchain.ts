 ...
 * @example
 * ```typescript
 * import { LangChainCostHandler } from '@traice/sdk/langchain';
 *
...
export class LangChainCostHandler {
  ...
  async handleLLMStart(_llm: any, _prompts: string[], runId: string): Promise<void> {
  ...
  async handleLLMEnd(output: any, runId: string): Promise<void> {
    ...
    const model = llmOutput.model ?? llmOutput.modelName ?? "unknown";
    let inputTokens = 0;
    let outputTokens = 0;
    ...
    const { inputCostUSD, outputCostUSD, totalCostUSD } = calculateCost(provider, model, inputTokens, outputTokens);

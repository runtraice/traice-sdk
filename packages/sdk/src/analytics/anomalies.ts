 ...
 */
export function detectAnomalies(events: CostEvent[], options: AnomalyOptions = {}): AnomalyResult[] {
  ...
  for (const [feature, dailyCosts] of byFeature) {
    const days = Array.from(dailyCosts.entries()).sort(([a], [b]) => a.localeCompare(b));

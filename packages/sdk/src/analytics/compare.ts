 ...
 */
export function comparePromptVersions(events: CostEvent[], promptName?: string): VersionComparison[] {
  ...
  for (const [key, groupEvents] of groups) {
    ...
    const avgInputTokens = Math.round(groupEvents.reduce((s, e) => s + e.inputTokens, 0) / calls);
    const avgOutputTokens = Math.round(groupEvents.reduce((s, e) => s + e.outputTokens, 0) / calls);
  ...
  for (const r of results) {
    ...
    if (baseline > 0) {
      r.changeFromBaseline = Math.round(((r.avgCostPerCall - baseline) / baseline) * 100);
    }
  }

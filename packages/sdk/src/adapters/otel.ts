 ...
 * @example
 * ```typescript
 * import { OTelAdapter } from "@traice/sdk";
...
export class OTelAdapter implements CostAdapter {
  ...
  constructor(config: OTelAdapterConfig = {}) {
    ...
    try {
      ...
      throw new Error("OTelAdapter requires @opentelemetry/api. Install it with: npm install @opentelemetry/api");
    }

    const meterName = config.meterName ?? "traice-sdk";

...
function mockAnthropicCachedResponse() {
  return async () => ({
    type: "message" as const,
    model: "claude-sonnet-4-20250514",
    usage: {
...
function mockOpenAICachedResponse() {
  return async () => ({
    object: "chat.completion",
    model: "gpt-4o",
    usage: {
...
describe("configure, resetConfig, and getConfig", () => {
  ...
  it("resetConfig() restores defaults", () => {
    ...
    expect(config.localPath).toBe("./.traice-costs/events.ndjson");
  });
...
describe("meter()", () => {
  ...
  it("normalizes Anthropic cache tokens as disjoint input buckets", async () => {
    await meter(mockAnthropicCachedResponse(), { feature: "test", awaitWrites: true });

    const event = testAdapter.events[0];
    expect(event.inputTokens).toBe(10_000);
    expect(event.outputTokens).toBe(500);
    expect(event.totalTokens).toBe(10_500);
    expect(event.cacheReadTokens).toBe(6000);
    expect(event.cacheWriteTokens).toBe(1000);
    expect(event.inputCostUSD).toBeCloseTo(0.01455, 6);
  });

  it("normalizes OpenAI cached tokens as a prompt-token subset", async () => {
    await meter(mockOpenAICachedResponse(), { feature: "test", awaitWrites: true });
    ...
    expect(event.inputCostUSD).toBeCloseTo(0.01875, 6);
  });
  ...
  it("records first-class workflow, customer, sample, and structured metadata options", async () => {
    await meter(mockAnthropicResponse(), {
      feature: "chat",
      userId: "user_42",
      tenantId: "tenant_1",
    ...
    expect(event.workflowId).toBe("workflow_1");
    ...
    expect(event.prompt).toBe("Prompt text");
    expect(event.output).toBe("Output text");
    expect(event.metadata).toEqual({ traceId: "trace_1" });
  });
...
describe("onError callback", () => {
  ...
  it("logs to console.error when verbose=true and no onError", async () => {
    ...
    expect(spy).toHaveBeenCalledWith("[trAIce SDK] Error writing event:", expect.any(Error));
    spy.mockRestore();
  });
});
...
describe("CostMeter class", () => {
  ...
  it("record() creates event from manual data", async () => {
    ...
    expect(event.workflowId).toBe("workflow_1");
    expect(event.runId).toBe("run_1");
    expect(event.stepId).toBe("manual-step");
    expect(event.toolName).toBe("manual");
    expect(event.retryCount).toBe(1);
    expect(event.outcome).toBe("success");
    expect(event.metadata).toEqual({ source: "record" });
...
describe("unknown model warning", () => {
  ...
  it("warns to console when warnOnMissingModel is true", async () => {
    ...
    expect(spy).toHaveBeenCalledWith(expect.stringContaining("claude-nonexistent-model"));
    spy.mockRestore();
  });

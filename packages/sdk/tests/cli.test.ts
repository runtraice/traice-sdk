...
describe("CLI report", () => {
  ...
  it("warns about malformed lines", () => {
    ...
    fs.writeFileSync(
      malformedFile,
      [JSON.stringify(sampleEvents[0]), "NOT VALID JSON", "{ broken", JSON.stringify(sampleEvents[1])].join("\n") +
        "\n",
    );

    const output = execSync(`node ${CLI_PATH} report --file ${malformedFile} --format json`, {
      encoding: "utf-8",

...
describe("calculateCost", () => {
  ...
  describe("edge cases", () => {
    it("applies Anthropic cache read discount and write premium", () => {
      const result = calculateCost("anthropic", "claude-sonnet-4-20250514", 10_000, 500, 6_000, 1_000);
      expect(result.inputCostUSD).toBeCloseTo(0.01455, 6);
      expect(result.outputCostUSD).toBeCloseTo(0.0075, 6);
      expect(result.totalCostUSD).toBeCloseTo(0.02205, 6);
    });

    it("applies OpenAI cache read discount without write premium", () => {
      const result = calculateCost("openai", "gpt-4o", 10_000, 500, 5_000, 0);
      expect(result.inputCostUSD).toBeCloseTo(0.01875, 6);
      expect(result.outputCostUSD).toBeCloseTo(0.005, 6);
      expect(result.totalCostUSD).toBeCloseTo(0.02375, 6);
    });

    it("clamps cache buckets to input tokens", () => {
      expect(normalizeCacheTokens(100, 90, 90)).toEqual({
...
describe("configurePricing", () => {
  it("adds custom model pricing", () => {
    configurePricing("openai", "ft:gpt-4o-mini:my-org", { input: 0.3, output: 1.2 });

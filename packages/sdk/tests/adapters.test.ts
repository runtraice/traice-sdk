...
describe("ConsoleAdapter", () => {
  ...
  it("logs formatted output", async () => {
    ...
    expect(output).toContain("[trAIce SDK]");
...
describe("LocalAdapter", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "traice-sdk-test-"));
  });
  ...
  it("handles concurrent writes without corruption", async () => {
    ...
    const writes = Array.from({ length: 20 }, (_, i) => adapter.write(makeMockEvent({ id: `concurrent-${i}` })));

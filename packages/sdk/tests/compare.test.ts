...
function makeEvent(
  ...
  return {
    id: `evt-${promptName}-${promptVersion}`,
    ...
    inputCostUSD: cost * 0.3,
    outputCostUSD: cost * 0.7,
    totalCostUSD: cost,
    latencyMs: 500,
    feature: "chat",
    promptName,
...
describe("comparePromptVersions", () => {
  ...
  it("filters by prompt name", () => {
    const events = [makeEvent("greeting", "v1", 0.01), makeEvent("summary", "v1", 0.05)];
  ...
  it("calculates average tokens", () => {
    const events = [makeEvent("greeting", "v1", 0.01, 100, 50), makeEvent("greeting", "v1", 0.01, 200, 100)];

...
describe("meterStream()", () => {
  ...
  it("records cost event after stream ends (OpenAI usage on stream object)", async () => {
    const stream = await meterStream(
      async () =>
        mockOpenAIStream([{ choices: [{ delta: { content: "Hi" } }] }], { prompt_tokens: 100, completion_tokens: 50 }),
      { feature: "chat", awaitWrites: true },
    );
  ...
  it("records cost event from Anthropic stream with message property", async () => {
    const stream = await meterStream(
      async () =>
        mockAnthropicStream([{ type: "content_block_delta", delta: { text: "Hi" } }], {
  ...
  it("extracts usage from OpenAI chunks (include_usage pattern)", async () => {
    ...
    const stream = await meterStream(async () => mockOpenAIStream(chunks), { feature: "test", awaitWrites: true });
  ...
  it("extracts usage from Anthropic chunk events", async () => {
    ...
    const stream = await meterStream(async () => mockAnthropicStream(chunks), { feature: "test", awaitWrites: true });
  ...
  it("records error event when stream throws", async () => {
    ...
    const stream = await meterStream(async () => failingStream as any, { feature: "chat", awaitWrites: true });
  ...
  it("measures latency across stream duration", async () => {
    ...
    const stream = await meterStream(async () => slowStream as any, { feature: "test", awaitWrites: true });
    for await (const _ of stream) {
      // Drain stream.
    }
...
describe("CostMeter.trackStream()", () => {
  it("works with instance config", async () => {
    ...
    const stream = await costMeter.trackStream(
      async () =>
        mockOpenAIStream([{ choices: [{ delta: { content: "Hi" } }] }], { prompt_tokens: 30, completion_tokens: 10 }),
      { feature: "chat", awaitWrites: true },
    );

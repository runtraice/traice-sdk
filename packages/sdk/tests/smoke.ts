...
const SMOKE_FILE = path.join(__dirname, "..", ".traice-costs", "smoke-test.ndjson");
...
async function testAnthropic(): Promise<void> {
  ...
  try {
    ...
    assert(response.content && response.content.length > 0, "Anthropic API returned a response");
...
async function testOpenAI(): Promise<void> {
  ...
  try {
    ...
    assert(response.choices && response.choices.length > 0, "OpenAI API returned a response");
